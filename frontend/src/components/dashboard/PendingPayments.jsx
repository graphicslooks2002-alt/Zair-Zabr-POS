            ...
            {rows.length === 0 ? (
              ...
              rows.map((p) => (
                <tr key={p._id} className="border-b border-surface">
                  ...
                  <td className="px-4 py-3 text-warn font-semibold">Rs{Number(p.pendingAmount).toFixed(0)}</td>
                  <td className="px-4 py-3 text-muted">{formatDateAndTime(p.orderDate)}</td>
