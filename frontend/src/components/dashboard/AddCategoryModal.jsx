    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[100] p-4">
      <div className="bg-[#262626] rounded-lg w-full max-w-sm p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-[#f5f5f5] text-lg font-semibold">Add Category</h2>
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="flex gap-2 items-center">
            <input value={icon} onChange={(e) => setIcon(e.target.value)} placeholder="🍕" maxLength={2}
              className="w-16 bg-[#1f1f1f] text-white rounded-lg px-2 py-2.5 text-sm outline-none text-center" />
            <label className="text-[#ababab] text-xs">Color</label>
            <input type="color" value={bgColor} onChange={(e) => setBgColor(e.target.value)}
              className="w-10 h-9 bg-[#1f1f1f] rounded-lg cursor-pointer" />
          </div>
