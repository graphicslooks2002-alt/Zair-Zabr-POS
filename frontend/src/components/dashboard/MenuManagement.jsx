...
const MenuManagement = () => {
  ...
  if (menu.length === 0) {
    return (
      <div className="container mx-auto py-10 px-6 text-center">
        <h2 className="text-main text-xl font-semibold mb-2">Menu is empty</h2>
        <p className="text-muted text-sm mb-5">Import the default menu to get started, then edit anything.</p>
        <button onClick={() => seedMut.mutate()} disabled={seedMut.isPending}
          className="bg-accent text-white px-6 py-3 rounded-lg font-semibold disabled:opacity-50">
  ...
  return (
    <div className="container mx-auto py-2 px-6">
      <h2 className="font-semibold text-main text-xl mb-1">Menu Management</h2>
      <p className="text-sm text-muted mb-4">Add, edit, remove categories and products. Every product belongs to a category.</p>

      <div className="grid grid-cols-3 gap-4">
        {/* CATEGORIES */}
        <div className="bg-panel rounded-lg p-4">
          <h3 className="text-main font-semibold mb-3">Categories</h3>
          <div className="space-y-1 max-h-[320px] overflow-y-auto scrollbar-hide">
            {menu.map((c) => (
              <div key={c._id}
                className={`flex items-center justify-between px-3 py-2 rounded-lg cursor-pointer ${activeCat?._id === c._id ? "bg-surface" : "hover:bg-[#222]"}`}
                onClick={() => setActiveCatId(c._id)}>
                <span className="text-main text-sm">{c.icon} {c.name}</span>
                <button
          ...
          {/* add category */}
          <div className="mt-3 border-t border-elevated pt-3 space-y-2">
            ...
            <div className="flex gap-2">
              <input value={catIcon} onChange={(e) => setCatIcon(e.target.value)} placeholder="Icon (emoji)" maxLength={2}
                className="w-16 bg-surface text-main rounded-lg px-2 py-2 text-sm outline-none text-center" />
              <input type="color" value={catColor} onChange={(e) => setCatColor(e.target.value)}
                className="w-10 h-9 bg-surface rounded-lg cursor-pointer" />
              <button onClick={() => catName.trim() ? addCatMut.mutate() : enqueueSnackbar("Enter a category name.", { variant: "warning" })}
                ...
                className="flex-1 bg-accent text-white rounded-lg text-sm font-semibold disabled:opacity-50 flex items-center justify-center gap-1">
                <FaPlus size={11} /> Add
              </button>
            </div>
          </div>
        ...
        {/* PRODUCTS */}
        <div className="bg-panel rounded-lg p-4 col-span-2">
          ...
          {activeCat && (
            <>
                  ...
                  <tbody className="text-main">
                    {activeCat.items.length === 0 ? (
                      ...
                      activeCat.items.map((p) => (
                        <tr key={p._id} className="border-b border-surface">
                          <td className="px-2 py-2">{p.name}</td>
                          <td className="px-2 py-2 text-right">Rs{p.price}</td>
                          <td className="px-2 py-2 text-right">
                            <div className="flex gap-2 justify-end">
                              <button onClick={() => startEdit(p)} className="text-info"><FaEdit /></button>
              ...
              {/* add / edit product */}
              <form onSubmit={submitProduct} className="mt-4 border-t border-elevated pt-3">
                <p className="text-muted text-xs mb-2">{editingProduct ? `Editing: ${editingProduct.name}` : `Add product to ${activeCat.name}`}</p>
                <div className="flex gap-2">
                  ...
                  <input type="number" min="0" value={prodPrice} onChange={(e) => setProdPrice(e.target.value)} placeholder="Price"
                    className="w-28 bg-surface text-main rounded-lg px-3 py-2 text-sm outline-none" />
                  <button type="submit" disabled={addProdMut.isPending || updProdMut.isPending}
                    className="bg-success text-white px-4 rounded-lg text-sm font-semibold disabled:opacity-50">
                    {editingProduct ? "Save" : "Add"}
                  </button>
                  {editingProduct && (
                    <button type="button" onClick={cancelEdit} className="bg-surface text-muted px-3 rounded-lg"><FaTimes /></button>
                  )}
                </div>
              </form>
            </>
