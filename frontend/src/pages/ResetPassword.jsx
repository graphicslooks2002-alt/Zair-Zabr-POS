...
const ResetPassword = () => {
  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-[#262626] rounded-lg p-8">
        <div className="flex flex-col items-center gap-2 mb-6">
          <img src={logo} alt="Zair Zabar" className="h-14 w-14 border-2 border-[#e85d04] rounded-full p-1" />
          <h1 className="text-xl font-bold text-[#e85d04]">Zair Zabar</h1>
        </div>
        <h2 className="text-2xl text-center font-semibold text-[#f5f5f5] mb-6">Reset Password</h2>

        {!token ? (
          <div className="text-center text-[#ababab]">
            <p>This reset link is invalid or missing.</p>
            <button onClick={() => navigate("/auth")} className="text-[#e85d04] font-semibold mt-4 hover:underline">Back to login</button>
          </div>
        ) : (
          <form onSubmit={submit} className="space-y-3">
            <div className="relative">
              <input type={show ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)}
                ...
                className="w-full bg-[#1f1f1f] text-white rounded-lg px-4 py-3 pr-10 outline-none" />
              <button type="button" onClick={() => setShow((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#ababab]">
            ...
            <input type={show ? "text" : "password"} value={confirm} onChange={(e) => setConfirm(e.target.value)}
              ...
              className="w-full bg-[#1f1f1f] text-white rounded-lg px-4 py-3 outline-none" />
            <button type="submit" disabled={mut.isPending}
              className="w-full bg-[#e85d04] text-white py-3 rounded-lg font-bold disabled:opacity-60">
